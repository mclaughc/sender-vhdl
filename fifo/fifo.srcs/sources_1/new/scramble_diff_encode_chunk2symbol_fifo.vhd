...
entity scramble_diff_encode_chunk2symbol_fifo is
...
end scramble_diff_encode_chunk2symbol_fifo;

architecture Behavioral of scramble_diff_encode_chunk2symbol_fifo is
...
main : process(reset, clk)
    ...
    variable scramble_res : std_logic;
    variable diff_encode_res : std_logic;
begin
    if (rising_edge(clk)) then
        if (reset = '0') then
            ...
            if (output_fifo_full = '0' and (has_input_value = '1' or j /= 0)) then
                ...
                scramble_res := pop(j) xor (shift(3) xor shift(6));
                diff_encode_res := last_out xor scramble_res;
                last_out <= diff_encode_res;
                
                -- insert the result on the corresponding output bit
                output_fifo_write_enable <= '1';
                if (diff_encode_res = '0') then
