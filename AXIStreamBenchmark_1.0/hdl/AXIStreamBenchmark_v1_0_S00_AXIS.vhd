...
entity AXIStreamBenchmark_v1_0_S00_AXIS is
	generic (
	...
	port (
        ...
        VALUE_COUNTER : out std_logic_vector(31 downto 0);
        LAST_VALUE : out std_logic_vector(C_S_AXIS_TDATA_WIDTH-1 downto 0)
	);
end AXIStreamBenchmark_v1_0_S00_AXIS;

architecture arch_imp of AXIStreamBenchmark_v1_0_S00_AXIS is
	...
	constant NUMBER_OF_INPUT_WORDS  : integer := 128;
    ...
    signal last_value_reg : std_logic_vector(C_S_AXIS_TDATA_WIDTH-1 downto 0);
    constant INCREMENT : integer := (C_S_AXIS_TDATA_WIDTH/8);
    
begin
	-- I/O Connections assignments
	LAST_VALUE <= last_value_reg;
    VALUE_COUNTER <= counter_reg;
	    ...
	    if(S_AXIS_ARESETN = '0') then
	      ...
	      if (write_pointer <= NUMBER_OF_INPUT_WORDS-1) then
	        if (fifo_wren = '1') then
	          ...
	          writes_done <= '0';
              counter_reg <= (counter_reg + std_logic_vector(to_signed(INCREMENT, 32)));
              last_value_reg <= S_AXIS_TDATA;
	        end if;
